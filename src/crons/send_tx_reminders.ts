...
const reminders: Map<number, NodeJS.Timeout> = new Map<number, NodeJS.Timeout>();
...
async function scheduleReminders() {
    try {
        ...
        relevantEtas
            ...
            .forEach((eta) => {
                const millisecondsUntilExecution = eta.diff(moment.now(), 'milliseconds');
                ...
                const timeoutId = setTimeout(async () => {
                    await sendMessage(
                        ...
                        )}! MasterChef multisig: https://safe.fantom.network/ftm:0x3Fa5c411857455e0E876412010BE600F4658Dddd/transactions/queue`,
                    );
